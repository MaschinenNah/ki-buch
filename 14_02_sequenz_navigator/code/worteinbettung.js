...
class Worteinbettung {
  ...
  constructor(wortZuVektorDict, wortZuVektorDict2D, metadaten, zeilen) {
    ...
    // alle Zeilen
    this.zeilen = zeilen;
    this.zeilenOhneDoppelte = undefined;
    // Anzahl Wörter
  ...
  initialisiere() {
    ...
    this.zeilenOhneDoppelte = [...new Set(this.zeilen)];

    GUI.berichtOben("Worteinbettung '" + GUI.einbettungsWaehler.value() + "' geladen.", true);
    GUI.berichtOben("Anzahl Zeilen in Textquelle: " +  this.zeilen.length);
    GUI.berichtOben("Anzahl Wörter in Textquelle: " +  this.nWoerter);
    GUI.berichtOben("Anzahl Vokabeln: " +  this.nVokabeln);
  ...
  // Schnittstelle: Liefert den dimensionsreduzierten Einbettungsvektor zu 
  // einem Wort. Falls es das Wort nicht gibt, wird der Nullvektor geliefert.
  // Falls das Argument bereits ein Vektor ist, wird der Vektor geliefert.
  zuVektor2D(wortOderVektor) {
    let resultat;
    
    if (typeof(wortOderVektor) == "string") {
      resultat = this.wortZuVektorDict2D[wortOderVektor];
      if (resultat) {
        return resultat
      } else {
        return Array(this.nDimensionen).fill(0)
  ...
  // TODO: Wird im Sequenz-Navigator nicht benötigt. raus?!
